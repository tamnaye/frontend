...
const BookingData = () => {
  ...
  function bookingdDataHandler(bookingData) {
    ...
    times.map((time) => arr.push(bookedTimes.includes(time) || checkPast(time) ? true : false));
    return arr;
  }
    ...
    if(roomType === roomTypeArr[0] && selectedNameState.length < 1 && getStartAndEndTime(checkedState).timeLength === 0){
      ...
      fetch(postUrl, {
        ...
        body: JSON.stringify({
          ...
          endTime: timePlusMinus(getStartAndEndTime(checkedState).startTime,getStartAndEndTime(checkedState).timeLength), // checked state에서  index 찾아서 times 배열에서 뽑아내서 +1
